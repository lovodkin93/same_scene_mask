...
def get_EAscenes(P):
    ...
    for c in C:
        if c!= '*':
            ...
            center.append(c)
            pos_center.append('*') #TODO: change here also pos_center.append(...)  - ask Elior what is the meaning of c=='*'
...
def get_difference(h1,L2,C2, pos_h1, pos_L2, pos_C2):
    ...
    for c2 in C2:
        if c2 == '*':
            c2 = [['#']]*len(h1) #TODO: ask Elior: what is done here? and do something parallel
    #print(C2)
...
def create_same_scene_map(pos_S1, sent_len):
...
def main():
    f1 = open(r'data\xml\1_0.xml')
    xml_string1 = f1.read()
    f1.close()
    f2 = open(r'data\txt\segmented1.txt')
    seg_sentence = f2.read()
    f2.close()
    ...
    same_scene_map=create_same_scene_map(pos_S1, sent_len) # TODO: the end game
    segmented_to_origin_map=create_segments_to_origin_map(seg_sentence, sent_len)

    same_scene_final_mask = np.matmul(segmented_to_origin_map, same_scene_map)
    same_scene_final_mask = np.matmul(same_scene_final_mask, np.transpose(segmented_to_origin_map))
    print("done!")
